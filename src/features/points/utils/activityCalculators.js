...
async function calculateActivityPoints(profileId, userId) {
  ...
  const workshopPoints =
    workshopParticipations.length * (rulesMap.WORKSHOP_PARTICIPATION || 50);
  ...
  const hackathonPoints =
    hackathonParticipations.length * (rulesMap.HACKATHON_PARTICIPATION || 100);
  ...
  const certificationPoints =
    approvedCertifications.length * (rulesMap.CERTIFICATION_APPROVED || 75);
  ...
  for (const stat of codingStats) {
    ...
    if (stat.platform === 'LEETCODE') {
      ...
      const problemBonus =
        Math.floor(stat.problemsSolved / 50) *
        (rulesMap.LEETCODE_PROBLEMS_BONUS || 5);
      ...
      const problemBonus =
        Math.floor(stat.problemsSolved / 50) *
        (rulesMap.HACKERRANK_PROBLEMS_BONUS || 3);
      platformPoints += problemBonus;
