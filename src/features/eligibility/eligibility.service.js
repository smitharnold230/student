...
async function checkEligibility(userId) {
  ...
  const leetcode = await CodingStat.findOne({
...
async function assignBatchesForAllEligibleStudents() {
  ...
  for (const student of students) {
    try {
      ...
      if (eligibility.eligible) {
        ...
        if (!student.Profile.batch) {
          ...
          results.push({
            userId: student.id,
            email: student.email,
            status: 'Batch assigned: PRODUCT',
          });
        } else {
          results.push({
            userId: student.id,
            email: student.email,
            status: `Already in batch: ${student.Profile.batch}`,
          });
        }
      } else {
        results.push({
      ...
      console.error(
        `Error processing batch for student ${student.id}:`,
        error.message,
      );
      results.push({
        userId: student.id,
...
module.exports = {
  checkEligibility,
  assignBatch,
  assignBatchesForAllEligibleStudents,
  BATCHES,
};
