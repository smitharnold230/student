...
async function uploadCertification(req, res, next) {
  try {
    ...
    const submission = await certificationService.uploadCertification(
      req.user.userId,
      eventId,
      fileUrl,
    );
    ...
    try {
      await notificationService.deleteNotificationByEventId(
        req.user.userId,
        eventId,
...
async function getUserCertifications(req, res, next) {
  try {
    const userCertifications = await certificationService.getUserCertifications(
      req.user.userId,
    );
...
async function verifyCertification(req, res, next) {
  try {
    ...
    const updated = await certificationService.verifyCertification(
      submissionId,
      status,
      req.user.userId,
    );
